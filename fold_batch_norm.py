...
#                print('bn_base:{}'.format(bn_base))
                if 'downsample' in layer:
#                    print('layer:{}'.format(layer))
                    conv_layer_num = int(bn_base.split('.')[-1]) - 1
#                    print('conv_layer_num:{}'.format(conv_layer_num))
                    conv_layer = '.'.join(bn_base.split('.')[:-1] + [str(conv_layer_num), 'weight'])
#                    print('conv_layer:{}'.format(conv_layer))
                else:
                    conv_layer = bn_base.replace('bn', 'conv') + '.weight'
                weight_layers.append(conv_layer)                    #把卷积的权重层挑出来
#                print('weight_layers:{}'.format(weight_layers))
...
def fold_batch_norm_for_one_layer(checkpoint, conv_layer, bn_base, var_eps=1e-5):
    ...
    # new bn variables: beta_new = beta - gamma * mean / sqrt(var + eps), gamma_new = 1, mean_new = 0, var_new = 1 - eps    checkpoint[bn_base + '.bias'] = bn_beta - bn_gamma * bn_mean * torch.rsqrt(torch.add(bn_var, var_eps))
